...
def get_untracked_songs(**context):
    ...
    cursor.execute(
        f"""SELECT DISTINCT spotify_id FROM listen
        where spotify_id IS NOT NULL and ts >= '{last_exec_ts}' ; """)
...
def get_spotify_token(**context):
    ...
    credentials = os.environ['SPOTIPY_CLIENT_ID'] + \
    ...
    context['ti'].xcom_push(key="spotify_token",
...
def get_songs_info(**context):
    ...
    keys_to_remove = ['analysis_url', 'type',
                      'uri', 'track_href', 'time_signature']
    features_list = []
    songs_list = []
    merged_li = []
